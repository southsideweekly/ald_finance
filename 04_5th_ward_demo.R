...
## 5th Ward
ward_5_candidates <- raw_ids %>% filter(ward_no == 5) %>% 
  ## merge in "other committees"
  bind_rows(data.frame(ward_no = 5, 
                       candidate = "Leslie Hairston",
                       candidate_committee = "Fifth Ward Regular Democratic Org",
                       committee_id = 14992))
ward_5_ids <- ward_5_candidates %>% 
...
file_names <- list.files("~/data/ald_finance/02_ward_demo/5th Ward/import")
receipts_raw <- lapply(file_names, function(file){
  read_csv(glue("~/data/ald_finance/02_ward_demo/5th Ward/import/{file}"),
           col_types = "nnnncccnnnccccccccccccccccccccc")
  }) %>% 
  bind_rows() %>% 
...
total_donations_plot <- total_donations %>% 
  ...
  labs(
    ...
    fill = "",
    caption = "
    Note: Total Receipts for Leslie Hairston include contributions to Fifth Ward Regular Democratic Org, which
    is chaired by Leslie Hairston. 
...
donor_stats %>% 
  ...
  tab_source_note(
    source_note = html("Note: Total Receipts for Leslie Hairston include contributions to Fifth Ward Regular Democratic Org, which
    is chaired by Leslie Hairston.<br>
    Source: illinoissunshine.org")
  ) %>% 
  tab_options(
    stub_group.font.weight = "bold"
...
walk(candidates, function(c){
  ...
  circle_plot <- ggplot() +
    ...
    labs(
      ...
      subtitle = glue("5th Ward - {curr_candidate}"),
      fill = "")
  
  # plot(circle_plot)
  circle_widget <- ggiraph(ggobj = circle_plot, width_svg = 10, height_svg = 10)
  htmlwidgets::saveWidget(widget = circle_widget, 
                          file = glue("~/data/ald_finance/02_ward_demo/5th Ward/Figure 4 Circle Widget - {curr_candidate}.html"))
})
...
circle_widget <- ggiraph(ggobj = circle_plot, width_svg = 10, height_svg = 10)
htmlwidgets::saveWidget(widget = circle_widget, 
                        file = glue("~/data/ald_finance/02_ward_demo/5th Ward/Figure 4 Circle Widget - all candidates.html"))
