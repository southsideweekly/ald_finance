...
## 25th Ward
ward_25_candidates <- raw_ids %>% filter(ward_no == 25) %>% 
  ## merge in "other committees"
  bind_rows(data.frame(ward_no = 25, 
                       candidate = "Hilario Dominguez",
                       candidate_committee = "Unite 25",
                       committee_id = 34652))
ward_25_ids <- ward_25_candidates %>% 
  filter(!is.na(committee_id)) %>% 
  distinct(committee_id) %>% 
  unlist()

## 02. Pull in donation data from Illinois Sunshine API ----
receipts_raw <- lapply(ward_25_ids, 
                       function(curr_id){
                         # curr_id = sw_ids[10]
